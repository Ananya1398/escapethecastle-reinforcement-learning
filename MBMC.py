...
def estimate_victory_probability(num_episodes=100000):
	...
	while (num_episodes > 0):
		...
		while not done:
			...
			if action == 4:
				if guard:
					fight_data[guard]['fights'] += 1
					if reward > 0:
						fight_data[guard]['wins'] += 1

		num_episodes -= 1

	for guard in fight_data:
		fights = fight_data[guard]['fights']
		wins = fight_data[guard]['wins']
		P[count] = wins/fights if fights > 0 else 0
		count += 1
