...
Complete the function below to do the following:
	...
	Some important notes:
		...
		- The value of alpha is unique for every (s,a) pair, and should be updated as 1/(1 + number of updates to Q(s,a)).
...
def Q_learning(num_episodes=10000, gamma=0.9, epsilon=1, decay_rate=0.999):
	...
	"""
	Q_table = {}
	update_count = np.zeros((375, env.action_space.n))

	for state in range(375):
	...
	for i in range(num_episodes):
		...
		while not done:
			...
			next_state_key = hash(obs)

			alpha = 1 / (1 + update_count[state_key][action])

			next_action = np.max(Q_table[next_state_key])

			Q_table[state_key][action] += alpha * (
						reward + gamma * next_action - Q_table[state_key][action])
			update_count[state_key][action] += 1
...
Q_table = Q_learning(num_episodes=1000000, gamma=0.9, epsilon=1,
					 decay_rate=decay_rate)  # Run Q-learning
